...
router.get('/dodajUBazu/:user/:pass/:email/:ime/:prezime/:drzava/:datum', function (req, res, next) {
    ...
    pool.getConnection(function (err, connection) {
        ...
        console.log("obicna: " + req.params.pass);
        var blokiran = 0;
        test = {};
        var podaci = {
            username: user,
...
router.get('/prikaziImanje/:idu', function (req, res, next) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query("select distinct p1.*,op.idOwner,u.username from user u join owner_parcel op on u.id=" + id + " and u.block=0  and u.id=op.idOwner join parcel p1 on op.idParcel=p1.id    union select distinct p1.*, op1.idOwner,u1.username  from user u join user_connect op on u.id=" + id + " and u.id=op.idWorker join owner_parcel op1 on op1.idOwner=op.idOwner join parcel p1 on op1.idParcel=p1.id join user u1 on u1.id=op.idOwner", function (err, rows) {
            connection.release();
            if (!err) {
                var test = [];
                for (var i = 0; i < rows.length; i++)
                    ...
                    connection.query("insert into specifications SET ?", podatak, function (err, rows) {
                        ...
                        if (!err) {
                            ...
                            connection.query("insert into subtype_spec SET ?", podatak1, function (err, rows) {
                                ...
                                if (!err)
                                    test.success = true;
                                else { console.log(err); test.success = false; }
                                res.json(test);
                            });
                        }
                        else {
...
router.get('/obrisiImanje/:id', function (req, res, next) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query("select id from plantation where id in (select idPlant from plant_parcel where idParcel=" + id + ")", function (err, rows) {
            if (err) {
                connection.release();
                console.log(err);
                test.success = false;
            ...
            connection.query("delete from plantation where id in (select idPlant from plant_parcel where idParcel=" + id + ")", function (err, rows) {
                if (!err) {
...
router.get("/dohvatiObavestenja", function (req, res, next) {
    ...
    pool.getConnection(function (err, connection) {
        ...
        if (sess.LoggedUser.id != undefined) {
            connection.query(`   select n.id as 'idnotif', u1.id as 'idworker', u2.id as 'idowner', r.id as 'idrole', u1.name as 'radnik', u2.name as 'ownername', u2.surname as 'ownerlastname', r.name as 'role', DATE_FORMAT(n.date,'%m-%d-%Y') as 'date',u2.profilePic, 0 as 'type'
        from notifications n join user u1 on u1.id=n.idWorker join user u2 on n.idOwner=u2.id join role r on r.id=n.idRole where u1.id= ` + sess.LoggedUser.id + ` 
        
         UNION
        
        select n.id as 'idnotif', u1.id as 'idworker', u2.id as 'idowner', r.id as 'idrole', u1.name as 'radnik', u2.name as 'ownername', u2.surname as 'ownerlastname', r.name as 'role', DATE_FORMAT(n.date,'%m-%d-%Y') as 'date',u2.profilePic, 1 as 'type'
        from worker_promotion n join user u1 on u1.id=n.idWorker join user u2 on n.idOwner=u2.id join role r on r.id=n.idRole where u1.id=` + sess.LoggedUser.id + `
...
router.get('/obrisiObavestenje/:id', function (req, res, next) {
    pool.getConnection(function (err, connection) {
        if (err) {
            res.json({ "code": 100, "status": "Error in connection database" });
            return;
        ...
        connection.query("delete from worker_promotion where id=" + req.params.id, function (err, rows) {
            var test;
            connection.release();
            if (!err) {
                test = true;
...
router.get('/getUserConnect', function (req, res, next) {
    pool.getConnection(function (err, connection) {
        if (err) {
            res.json({ "code": 100, "status": "Error in connection database" });
            return;
        ...
        connection.query("select * from user_connect", function (err, rows) {
            var test;
            connection.release();
            if (!err) {
                res.json(rows);
...
router.get('/updateWorkers/:workerID/:roleID', function (req, res, next) {
    ...
    pool.getConnection(function (err, connection) {
        ...
        var roleID = req.params.roleID;
        console.log(id);
        connection.query("INSERT INTO worker_promotion(date, idWorker, idOwner, idRole) select NOW(), idWorker, idOwner, " + roleID + " as 'idRole' from user_connect where id=" + id, function (err, rows) {
            if (err) {
                console.log(err);
                return;
            }
...
router.get('/vratiFAQ', function (req, res, next) {
    pool.getConnection(function (err, connection) {
        if (err) {
            res.json({ "code": 100, "status": "Error in connection database" });
            return;
